...
function countAllarticle() {
	...
	$rows = $db->select();
		foreach($rows as $row){
			$db->table = "article";
		    $db->condition = "article_menu_id =".$row['article_menu_id'];
		    $db->order = "";
		    $db->limit = "";
		    $rows2 = $db->select();
            $soluong = $soluong + $db->RowCount;
		}
		return $soluong;
    }


function setAllarticle() {
	global $db;
	$content = '';
	$db->table = "article_menu";
	$db->condition = "category_id = 77";
...
function getAllarticle($limit,$start) {
	...
	$rows = $db->select();
	 ...
	 if($limit == 0)
	 {
	 for($i = $limit ; $i < $limit + $start ; $i++){
         $mangk = $mangds[$i];
          if(isset($mangk))
          array_push($sophantu, $mangk);
	 }
	}
	 else if($limit != 0)
	 {
...
function videodau(){
	...
	$content = '';
	$db->table = "gallery";
	$db->condition = "gallery_menu_id=83";
	$db->order = "gallery_id desc";
	$db->limit = "1";
	$rows = $db->select();
	return $rows;
...
function dsvideokhac($id){
	global $db;
	$content = '';
	$db->table = "gallery";
	$db->condition = "gallery_id !=".$id." and gallery_menu_id=83";
	$db->order = "gallery_id desc";
	$db->limit = "12";
	$rows = $db->select();
	return $rows;
...
function getLimitArticleItem($id,$limit,$start)
{
global $db;
	$content = '';
	$db->table = "article";
...
// 	$str = "";
// 	$db->table = "article_menu";
// 	$db->condition = "article_menu_id = 361";
// 	$db->order = "";
// 	$db->limit = "";
// 	$rows = $db->select();
// 	foreach ($rows as $row){
