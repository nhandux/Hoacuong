...
function send_res(frmContactContact)
{
	...
	if(!txtName||txtName=="Họ và tên (*)")
	{
		alert("Vui lòng nhập họ tên !");
		frmContactContact.txtName.value="";
		frmContactContact.txtName.focus();
		return false;
	}
	if(txtEmail=="E-mail (*)"||!txtEmail || !txtEmail.match(/^([-\d\w][-.\d\w]*)?[-\d\w]@([-\w\d]+\.)+[a-zA-Z]{2,6}$/) )
	{
	...
	if(!txtSubject||txtSubject=="Chủ đề (*)")
	{
	...
	if(!txtContent||txtContent=="Nội dung (*)")
	{
		alert("Vui lòng nhập nội dung !");
		frmContactContact.txtContent.value="";
...
{    
	/*var listID = ['url','price','product_name','name','email','address','phone'];
	var length =listID.length;
	for(var i=0;i<length;i++)
	{
		listID[i]=$('#txt_'+listID[i]).val();
	}*/
	...
	if(name=='')
	{
		$('#luu_result').html('<span>Vui lòng nhập tên</span>');
		$('#txt_name').focus();
		return false;
	}
	if(email=='')
	{
		$('#luu_result').html('<span>Vui lòng nhập email</span>');
		$('#txt_email').focus();
		return false;
	}
	else
	{
		if (!email.match(/^([-\d\w][-.\d\w]*)?[-\d\w]@([-\w\d]+\.)+[a-zA-Z]{2,6}$/))
		{
			$('#luu_result').html('<span>Vui lòng nhập đúng định dạng email!</span>');
			$('#txt_email').focus();
			return false;
		}
	}
	if(address=='')
	{
		$('#luu_result').html('<span>Vui lòng nhập Địa chỉ</span>');
		$('#txt_address').focus();
		return false;
	}
	if(phone=='')
	{
		$('#luu_result').html('<span>Vui lòng nhập số điện thoại</span>');
		$('#txt_phone').focus();
		return false;
	}
    var data="sendOrder=1&product_name="+product_name+"&url="+url+"&price="+price+"&name="+name+"&email="+email+"&address="+address+"&phone="+phone+"&note="+note;
    $.ajax({
		    url: 'index.php',
			type: "POST",
 			data: data,
 			success: function (html) {
				{
					if(html)
					{
						if(html==1)
						{
							$('#luu_result').html("<span>Đã gửi đơn đặt hàng thành công ! Vui lòng kiểm tra email của bạn !</span>");
							setInterval(location.reload(true), 5000);
						}
						else
						$('#luu_result').html(html);
					}
					else
						alert("Có lỗi xảy ra!");
				}
