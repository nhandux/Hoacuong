...
if($id_menu > 0) {
	$parent = 0;
	$db->table = "tour_menu";
	$db->condition = "tour_menu_id = " . $id_menu;
	$db->order = "";
	$db->limit = 1;
	$rows = $db->select();
	if($db->RowCount>0) {
		foreach ($rows as $row) {
			...
			if ($parent == 0) {
				...
				$db->table = "tour_menu";
				$db->condition = "tour_menu_id = " . $parent;
				$db->order = "";
				$db->limit = 1;
...
echo '<div class="breadcrumb link-tour">' . $breadcrumb_home . $breadcrumb_category . $breadcrumb_menu_parent . $breadcrumb_menu . $breadcrumb_detail . '</div>';
echo '<section class="content clearfix">
<div class="content-left">';
//-------------------------------------------------------------------------------
if ($id_article > 0){
	$id = $id_article;
	include(_F_TEMPLATES . DS . "show_tour.php");
} else if($id_menu <= 0) {
	$loc = array();
	$db->table = "tour_menu";
	...
	$stt=0;
	foreach($rows as $row) {
		$loc[$stt] = $row['tour_menu_id'];
		$stt++;
	}
	$loc = implode(',',$loc);
	$db->table = "tour";
	$db->condition = "is_active = 1 and tour_menu_id IN (".$loc.")";
	...
	if($total>1) {
		...
		foreach($rows as $row) {
			include(_F_TEMPLATES . DS . "show_list_tour.php");
			$i++;
		}
        echo '</div>';
	...
	$parent = false;
	$db->table = "tour_menu";
	$db->condition = "is_active = 1 and tour_menu_id = ".$id_menu;
	$db->order = "";
	$db->limit = 1;
	$rows = $db->select();
	foreach($rows as $row) {
